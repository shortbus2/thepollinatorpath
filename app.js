...
(function(){
 ...
 function card(p){return `<a class="plant-card" href="plant.html?id=${p.number}"><div class="plant-photo"><span class="plant-number">#${p.number}</span><span>${iconFor(p)}</span></div><div class="plant-content"><h3>${p.common}</h3><div class="botanical">${p.botanical}</div><div class="meta">${p.bloom}</div><div class="chips"><span class="chip">${p.status}</span><span class="chip">${p.type}</span></div><div class="card-link">View plant →</div></div></a>`}
 ...
 if(page){
   ...
   page.innerHTML=`<div class="plant-layout"><div class="plant-hero-image"><span class="plant-number">Plant #${p.number}</span>${iconFor(p)}</div><div class="plant-header"><div class="standing">📍 You’re standing in front of this plant—or you found it from the map while avoiding eye contact with a spiderweb.</div><h1>${p.common}</h1><div class="botanical" style="font-size:1.25rem">${p.botanical}</div><div class="chips"><span class="chip">${p.status}</span><span class="chip">${p.type}</span>${p.pollinators.map(x=>`<span class="chip">${x}</span>`).join('')}</div><div class="quick-grid"><div class="quick">☀️ <strong>Light</strong><small>${p.sun}</small></div><div class="quick">💧 <strong>Water</strong><small>${p.water}</small></div><div class="quick">🌸 <strong>Blooms</strong><small>${p.bloom}</small></div><div class="quick">📍 <strong>Find it</strong><small>${p.mapZone}</small></div></div></div>
