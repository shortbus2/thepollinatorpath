(()=>{
  ...
  const visitors=visitorIds.map(id=>(window.VISITORS||[]).find(v=>v.slug===id)?.name||(window.GARDEN_RESIDENTS||[]).find(r=>r.id===id)?.name||id).filter(Boolean);
  const plants=plantNums.map(n=>(window.PLANTS||[]).find(p=>Number(p.number)===n)?.common).filter(Boolean);
  ...
  summary.textContent=(parts.length?parts.join('. ')+'. ':'')+'Open the walks to follow the photographs, observations, and stories down their connected rabbit holes.';
  chips.innerHTML=[...visitors.slice(0,4).map(x=>`🐝 ${x}`),...plants.slice(0,4).map(x=>`🌿 ${x}`)].map(x=>`<span class="weekly-chip">${x}</span>`).join('');
})();
